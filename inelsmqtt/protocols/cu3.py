...
class DT_100:
    ...
    def create_ha_value_object(cls, device_value: DeviceValue) -> Any:
        ...
        relay_overflow = []
        relay_overflow.append(int(trim_inels_status_values(device_value.inels_status_value, cls.DATA, RELAY_OVERFLOW, ""),16) == 1)
...
class DT_101(Base):
    ...
    def create_ha_value_object(cls, device_value: DeviceValue) -> Any:
        ...
        state = trim_inels_status_values(
            device_value.inels_status_value, cls.DATA, DA3_22M, "")
        ...
        out1 = int(
            trim_inels_status_values(
                device_value.inels_status_value, cls.DATA, DIM_OUT_1, ""
            ), 16
        )

        out2 = int(
            trim_inels_status_values(
                device_value.inels_status_value, cls.DATA, DIM_OUT_2, ""
            ), 16
        )
...
class DT_103(Base):
    ...
    @classmethod
    def create_ha_value_object(cls, device_value: DeviceValue) -> Any:
        digital_inputs = trim_inels_status_values(
            device_value.inels_status_value, cls.DATA, GSB3_90SX, "")
        digital_inputs = f"0x{digital_inputs}"
        digital_inputs = f"{int(digital_inputs, 16):0>16b}"
        
        temp = trim_inels_status_values(
            device_value.inels_status_value, cls.DATA, TEMP_IN, "")

        light_in = trim_inels_status_values(
            device_value.inels_status_value, cls.DATA, LIGHT_IN, "")

        ain = trim_inels_status_values(
            device_value.inels_status_value, cls.DATA, AIN, "")

        humidity = trim_inels_status_values(
            device_value.inels_status_value, cls.DATA, HUMIDITY, "")

        dewpoint = trim_inels_status_values(
            device_value.inels_status_value, cls.DATA, DEW_POINT, "")
...
class DT_105(DT_100):
    ...
    def create_ha_value_object(cls, device_value: DeviceValue) -> Any:
        ...
        digital_inputs = trim_inels_status_values(
            device_value.inels_status_value, cls.DATA, SW, "")
        digital_inputs = f"0x{digital_inputs}"
...
class DT_109(Base):
    ...
    def create_ha_value_object(cls, device_value: DeviceValue) -> Any:
        ...
        alerts = trim_inels_status_values(
            device_value.inels_status_value, cls.DATA, RELAY_OVERFLOW, ""
        )
        alerts = f"0x{alerts}"
...
class DT_111:
    ...
    def create_inels_set_value(cls, device_value: DeviceValue) -> str:
        ...
        set_val.extend(
            [cls.RELAY_SET[device_value.ha_value.fan_speed == (i + 1)] for i in range(3)]
        )
...
class DT_112(DT_100):
    ...
    def create_ha_value_object(cls, device_value: DeviceValue) -> Any:
        ...
        digital_inputs = trim_inels_status_values(
            device_value.inels_status_value, cls.DATA, DIN, "")
        digital_inputs = f"0x{digital_inputs}"
        digital_inputs = f"{int(digital_inputs, 16):0>8b}"
...
class DT_114(Base):
    ...
    def create_ha_value_object(cls, device_value: DeviceValue) -> Any:
        ...
        temp_bytes = trim_inels_status_bytes(
            device_value.inels_status_value, cls.DATA,
            TEMP_IN,
        )
        ...
        relay_overflow=[]
        overflows = trim_inels_status_values(
            device_value.inels_status_value, cls.DATA, RELAY_OVERFLOW, "")
        overflows = f"0x{overflows}"
        overflows = f"{int(overflows, 16):0>8b}"
        for i in range(len(re)):
        ...
        aout_coa = []
        alerts = trim_inels_status_values(
            device_value.inels_status_value, cls.DATA, ALERT, "")
        alerts = f"0x{alerts}"
        ...
        dali_raw = trim_inels_status_bytes(
            device_value.inels_status_value, cls.DATA, DALI)
        dali = []
        for d in dali_raw:
            dali.append(
                DALILight(
...
class DT_136(Base):
    ...
    @classmethod
    def create_ha_value_object(cls, device_value: DeviceValue) -> Any:
        switches = trim_inels_status_values(
            device_value.inels_status_value, cls.DATA, SW, "")
        switches = f"0x{switches}"
        switches = f"{int(switches, 16):0>8b}"
...
class DT_151:
    ...
    def create_ha_value_object(cls, device_value: DeviceValue) -> Any:
        ...
        out = []
        outs = trim_inels_status_bytes(
            device_value.inels_status_value, cls.DATA, OUT
        )
...
class DT_166:
    ...
    def create_ha_value_object(cls, device_value: DeviceValue) -> Any:
        ...
        if temp_current == 0x7FFFFFFB:
            ...
            temp_current /= 100
        temp_critical_max = int(trim_inels_status_values( #check if 0x7F FF FF FB -> make it 50
            device_value.inels_status_value, cls.DATA, CRITICAL_MAX_TEMP, ""
        ), 16) / 100
        temp_required_heat = int(trim_inels_status_values(device_value.inels_status_value, cls.DATA, REQUIRED_HEAT_TEMP, ""), 16)
        if temp_required_heat == 0x7FFFFFFB:
            temp_required_heat = 0
        else:
            temp_required_heat /= 100
        temp_critical_min = int(trim_inels_status_values( #check if 0x7F FF FF FB -> make it -50
            device_value.inels_status_value, cls.DATA, CRITICAL_MIN_TEMP, ""
        ), 16) / 100
        temp_required_cool = int(trim_inels_status_values(device_value.inels_status_value, cls.DATA, REQUIRED_COOL_TEMP, ""), 16)
        if temp_required_cool == 0x7FFFFFFB:
            temp_required_cool = 0
        else:
            temp_required_cool /= 100
        temp_correction = int(trim_inels_status_values(device_value.inels_status_value, cls.DATA, TEMP_CORRECTION, ""), 16) / 100
    ...
    def create_inels_set_value(cls, device_value: DeviceValue) -> str:
        ...
        manual_cool_temp = [int(x, 16) for x in break_into_bytes(f"{int((cc.required_cool + cc.correction_temp) * 100):08X}")]
